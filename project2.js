...
// Project 1 code for cube movements
...
var roughness = 60
var height = 5
var cubeOffset = 1 // for initial height positioning of the shapes
var g_sphereMesh

// User-controlled sphere position
var g_sphereSpeed = 0.3
...
// Terrain Stuff (Adopted from Demo)
var width = 100
var depth = 100
...
// slice the positions down to help with interraction detectability for small movements
for (let i = 0; i < terrain.length; i++) {
    terrain[i] = terrain[i].map((component) => component * 0.2)
}
// send data to terrainMesh
var g_terrainMesh = []
...
var g_sphereVelocityX = 0
var g_sphereTargetVelocityX = 0
var g_sphereAcceleration = 0.01
var g_sphereDeceleration = 0.95
...
async function loadOBJFiles() {
    const ballData = await fetch("./resources/ball.obj").then((response) => response.text())
...
function tick() {
    ...
    const deltaTime = (currentTime - g_lastFrameMS) / 1000 // seconds
...
function moveCubes(deltaTime) {
    ...
    // Moving the Cube, alternating translations
    ...
    // Independent X position for arm1, using a different oscillation pattern
    const arm1XPosition = maxCubeMove * Math.sin(Date.now() * 0.001 * moveSpeed + 1.5)
    arm1_transform.translate(arm1XPosition, 0.0, 0.0)
    arm1_transform.rotate(armRotationAngle, 0, 1, 0) // rotating on y for vertical spin
    g_arm1ModelMatrix = arm1_transform

    // Rotate Arm 2 still attached to cube
    const arm2_transform = new Matrix4()
    arm2_transform.set(g_cubeModelMatrix)
    arm2_transform.translate(0.1, 0.0, 0.0)
    arm2_transform.rotate(-armRotationAngle, 0, 1, 0) // opposite to the other arm
    arm2_transform.translate(-0.1, 0.0, 0.0)
    g_arm2ModelMatrix = arm2_transform


    // Velocity affected by both acceleration and deceleration
    g_sphereVelocityX = g_sphereVelocityX * g_sphereDeceleration + g_sphereTargetVelocityX * g_sphereAcceleration

    const sphere_transform = new Matrix4()
    sphere_transform.translate(g_sphereVelocityX, 0, 0)
    g_sphereModelMatrix = sphere_transform.multiply(g_sphereModelMatrix)

    // Reset target velocity (will be set by key presses)
    g_sphereTargetVelocityX = 0
}
...
function init() {
    ...
    const sphere_transform = new Matrix4()
    sphere_transform.translate(-0.1, 0.8, 0).setScale(0.2, 0.2, 0.2) // Make the sphere a bit smaller
...
document.addEventListener("keydown", (event) => {
    switch (event.key) {
        ...
        case "d":
            ...
            break
        // User-controlled sphere movement - now setting target velocity
        case "l":
            g_sphereTargetVelocityX = -g_sphereSpeed // Move sphere right
            break
        case "j":
            g_sphereTargetVelocityX = g_sphereSpeed // Move sphere left
            break
    }
})
...
// get height of terrain for any x value --  my z is constant at 10
...
// Only look at a portion of the terrain that is nearby
const epsilon = 0.1
let filteredTerrain = terrain.filter((point) => Math.abs(point[2] - 10) <= epsilon)

function handleCubeTerrain() {
    const modelX = g_cubeModelMatrix.elements[12] + 10 // Cube X position, I'll keep Z at 10
