...
import {
  ...
  function startRendering() {
    ...
    if (
      !setupVec3(
        ...
        "a_Color",
        0,
        (g_cubeMesh.length + g_arm1Mesh.length + g_arm2Mesh.length + g_terrainMesh.length) * FLOAT_SIZE,
      )
    )
      return
  ...
  function draw() {
    ...
    gl.drawArrays(gl.TRIANGLES, (g_cubeMesh.length + g_arm1Mesh.length + g_arm2Mesh.length) / 3, g_terrainMesh.length / 3)
  }
  ...
  function regen() {
    ...
    const offset = FLOAT_SIZE * (g_cubeMesh.length + g_arm1Mesh.length + g_arm2Mesh.length)
    gl.bufferSubData(gl.ARRAY_BUFFER, offset, new Float32Array(g_terrainMesh))
    const offset2 =
      offset + FLOAT_SIZE * (g_terrainMesh.length + cubeColors.length + arm1Colors.length + arm2Colors.length)
